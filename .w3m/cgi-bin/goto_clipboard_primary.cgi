...
# Capture the content of the primary clipboard
CLIPBOARD_CONTENT=$(wl-paste --primary)

# Handle empty clipboard content
if [ -z "$CLIPBOARD_CONTENT" ]; then
  echo "No URL found in the primary clipboard. Exiting..."
  exit 1
fi

# Print the W3m-control GOTO command with the clipboard content
printf "%s\r\n" "W3m-control: GOTO $CLIPBOARD_CONTENT"

# Delete the buffer (element in history) created between the current page and the searched page by calling this script.
printf "W3m-control: DELETE_PREVBUF\r\n"
